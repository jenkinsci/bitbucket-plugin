			...
			try {
				...
				for (Job<?, ?> job : Jenkins.getInstance().getAllItems(Job.class)) {
					...
					if (bTrigger != null) {
						...
						Collection<? extends SCM> scMs = item.getSCMs();
						if (scMs.isEmpty())
							LOGGER.log(Level.INFO, "No SCM is set up for: {0}", job.getName());
						for (SCM scmTrigger : scMs) {
							if (match(scmTrigger, remote) && !hasBeenTriggered(scmTriggered, scmTrigger)) {
								...
								LOGGER.log(Level.INFO, job.getName() + " SCM doesn't match remote repo " + remote);
						}
					} else
